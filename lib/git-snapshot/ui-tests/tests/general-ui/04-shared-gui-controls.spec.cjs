...
async function postGuiJson(url, data, options) {
  ...
  const retries = Number.isFinite(requestOptions.retries) && requestOptions.retries >= 0
    ...
    : 1;
  const retryDelayMs = Number.isFinite(requestOptions.retryDelayMs) && requestOptions.retryDelayMs >= 0
  ...
  throw lastError || new Error(`POST ${url} failed.`);
}
...
async function resetViewedStateForTest(page, guiUrl, repoPath, snapshotHome, mode = "browse") {
  ...
  const clearResponse = await postGuiJson(clearUrl.toString(), { scope: "all" });
...
test("browse changed-since-viewed rows expose current versus since-viewed preview and current-mode clear", async ({ page }) => {
  ...
  const rowLabel = `${targetFile} [untracked]`;
  const markResponse = await postGuiJson(
