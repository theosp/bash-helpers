...
async function expectCompareLoadFailure(page) {
...
test("compare data load failures render a real inline error state", async ({ page }) => {
  ...
  const listMessage = await expectCompareLoadFailure(page);
  ...
  const refreshResponsePromise = page.waitForResponse((response) => {
    return response.request().method() === "GET" && response.url().includes("/api/data?");
  });
  await page.locator("#refresh").click();
  const refreshResponse = await refreshResponsePromise;
  expect(refreshResponse.ok()).toBeFalsy();
  await expectCompareLoadFailure(page);
