...
async function expectSuccessfulAutoDetect(page, availability, expectedTool) {
  ...
  if (expectedTool === "code") {
    ...
    expect(fs.readFileSync(logFields.arg_1, "utf8")).toContain("diverged line 001");
    expect(fs.readFileSync(logFields.arg_2, "utf8")).not.toContain("diverged line 001");
    ...
    expect(fs.existsSync(logFields.arg_1)).toBeTruthy();
    expect(fs.readFileSync(logFields.arg_0, "utf8")).toContain("diverged line 001");
    expect(fs.readFileSync(logFields.arg_1, "utf8")).not.toContain("diverged line 001");
  }
}
...
test("open external diff auto-detect flips built-in ordering when compare_base=snapshot", async ({ page }) => {
  const { guiUrl, logFile, spawnLogFile, fakeBinDir } = getGuiTestEnv();
  setToolAvailability(fakeBinDir, { meld: true, kdiff3: true, opendiff: true, bcompare: true, code: true });
  ...
  expect(fs.readFileSync(logFields.arg_0, "utf8")).not.toContain("diverged line 001");
  expect(fs.readFileSync(logFields.arg_1, "utf8")).toContain("diverged line 001");
