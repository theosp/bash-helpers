...
assert_contains $'reset_all_snapshot\tcreated=true\tsnapshot_id=before-reset-all-' "${porcelain_snapshot_output}" "snapshot porcelain row should report created=true and generated id"
assert_contains $'reset_all_summary\tresult=success\tsnapshot_created=true\tsnapshot_id=before-reset-all-' "${porcelain_snapshot_output}" "summary row should report successful clear with snapshot"
...
assert_not_contains "before-reset-all-" "${porcelain_no_snapshot_output}" "no-snapshot porcelain output should not include generated snapshot id"
