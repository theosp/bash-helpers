...
mode_porcelain_output="$(cd "${repo}" && git_snapshot_test_cmd compare "${mode_snapshot_id}" --repo . --include-no-effect --porcelain)"
...
mode_committed_porcelain_output="$(cd "${repo}" && git_snapshot_test_cmd compare "${mode_snapshot_id}" --repo . --include-no-effect --porcelain)"
...
symlink_porcelain_output="$(cd "${repo}" && git_snapshot_test_cmd compare "${symlink_snapshot_id}" --repo . --include-no-effect --porcelain)"
...
symlink_committed_porcelain_output="$(cd "${repo}" && git_snapshot_test_cmd compare "${symlink_snapshot_id}" --repo . --include-no-effect --porcelain)"
...
assert_contains "Compare: restore would change paths in the current workspace." "${symlink_diverged_output}" "worktree divergence should keep compare unresolved"
assert_contains "link.txt [binary change]" "${symlink_diverged_output}" "diverged symlink should render as a simplified non-text change"

symlink_diverged_porcelain_output="$(cd "${repo}" && git_snapshot_test_cmd compare "${symlink_snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'compare_file\tsnapshot_id='"${symlink_snapshot_id}"$'\trepo=.\tfile=link.txt\tstatus=unresolved_diverged\treason=current content or mode diverges from snapshot target\tpath_scope=both\trestore_effect=changes\tlines_added=-\tlines_removed=-\tdisplay_kind=binary_change\tdisplay_label=binary change' "${symlink_diverged_porcelain_output}" "porcelain should expose simplified display metadata for symlink divergence"
assert_contains $'compare_summary\tsnapshot_id='"${symlink_snapshot_id}"$'\trepos_checked=1\tfiles_total=1\tresolved_committed=0\tresolved_uncommitted=0\tunresolved_missing=0\tunresolved_diverged=1\tunresolved_total=1\tshown_files=1\tscope_both=1\tscope_snapshot_only=0\tscope_current_only=0\tengine=v3\telapsed_ms=' "${symlink_diverged_porcelain_output}" "porcelain summary should keep v6 counters for diverged symlink"
assert_contains $'\tcontract_version=8' "${symlink_diverged_porcelain_output}" "porcelain summary should expose v8 contract version"
