...
assert_contains "Auto: *" "${auto_details}" "auto snapshot details should include marker"
user_details="$(printf "%s\n" "${list_all_output}" | awk -v sid="${snapshot_id}" '$0 == sid {getline; print; exit}')"
assert_not_contains "Auto: *" "${user_details}" "user snapshot details should not include auto marker"
