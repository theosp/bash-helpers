...
first_compare="$(cd "${repo}" && git_snapshot_test_cmd compare "${snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'compare_summary\tsnapshot_id='"${snapshot_id}"$'\trepos_checked=1\tfiles_total=1\tresolved_committed=0\tresolved_uncommitted=1\tunresolved_missing=0\tunresolved_diverged=0\tunresolved_total=0\tshown_files=1\tscope_both=1\tscope_snapshot_only=0\tscope_current_only=0\tengine=v3\telapsed_ms=' "${first_compare}" "first compare should emit v6 summary"
assert_contains $'\tcache_hit_repos=0\tcache_miss_repos=1\t' "${first_compare}" "first compare should populate cache miss telemetry"
assert_contains $'\tcontract_version=8\t' "${first_compare}" "first compare should expose the v8 compare contract"

second_compare="$(cd "${repo}" && git_snapshot_test_cmd compare "${snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'\tcache_hit_repos=1\tcache_miss_repos=0\t' "${second_compare}" "second compare should reuse persistent cache"
assert_contains $'\tcontract_version=8\t' "${second_compare}" "second compare should expose the v8 compare contract"

first_compare_human="$(cd "${repo}" && git_snapshot_test_cmd compare "${snapshot_id}" --repo . --include-no-effect)"
...
status_invalidation_human="$(cd "${repo}" && printf "status-invalidation-human\n" >> tracked.txt && git_snapshot_test_cmd compare "${snapshot_id}" --repo . --include-no-effect)"
...
status_invalidation_compare="$(cd "${repo}" && git_snapshot_test_cmd compare "${snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'\tcache_hit_repos=0\tcache_miss_repos=1\t' "${status_invalidation_compare}" "status-hash changes should invalidate cache"

status_warm_compare="$(cd "${repo}" && git_snapshot_test_cmd compare "${snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'\tcache_hit_repos=1\tcache_miss_repos=0\t' "${status_warm_compare}" "unchanged status after invalidation should warm cache again"
...
same_shape_first="$(cd "${same_shape_repo}" && git_snapshot_test_cmd compare "${same_shape_snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'\tcache_hit_repos=0\tcache_miss_repos=1\t' "${same_shape_first}" "first same-shape compare should miss cache"
...
printf "snapshot-two\n" > "${same_shape_repo}/tracked.txt"
same_shape_second="$(cd "${same_shape_repo}" && git_snapshot_test_cmd compare "${same_shape_snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'\tcache_hit_repos=0\tcache_miss_repos=1\t' "${same_shape_second}" "content-only tracked edits should invalidate cache even when status shape stays stable"
...
same_untracked_first="$(cd "${same_untracked_repo}" && git_snapshot_test_cmd compare "${same_untracked_snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'\tcache_hit_repos=0\tcache_miss_repos=1\t' "${same_untracked_first}" "first untracked compare should miss cache"
...
same_untracked_second="$(cd "${same_untracked_repo}" && git_snapshot_test_cmd compare "${same_untracked_snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'\tcache_hit_repos=0\tcache_miss_repos=1\t' "${same_untracked_second}" "content-only untracked edits should invalidate cache"
...
# 1d) Dirty paths outside the captured snapshot subset still participate in root-scope compare.
...
targeted_first="$(cd "${targeted_cache_repo}" && git_snapshot_test_cmd compare "${targeted_snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'\tcache_hit_repos=0\tcache_miss_repos=1\t' "${targeted_first}" "first targeted compare should miss cache"
...
targeted_second="$(cd "${targeted_cache_repo}" && git_snapshot_test_cmd compare "${targeted_snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'\tcache_hit_repos=0\tcache_miss_repos=1\t' "${targeted_second}" "root-scope compare should invalidate cache when a new dirty path appears"
assert_contains $'compare_file\tsnapshot_id='"${targeted_snapshot_id}"$'\trepo=.\tfile=unrelated.txt\tstatus=unresolved_diverged\treason=current-only dirty path diverges from restore baseline\tpath_scope=current_only' "${targeted_second}" "root-scope compare should include unrelated dirty paths as current-only rows"
...
special_first="$(cd "${special_path_repo}" && git_snapshot_test_cmd compare "${special_snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'\tcache_hit_repos=0\tcache_miss_repos=1\t' "${special_first}" "first special-path compare should miss cache"
...
special_second="$(cd "${special_path_repo}" && git_snapshot_test_cmd compare "${special_snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'\tcache_hit_repos=1\tcache_miss_repos=0\t' "${special_second}" "second special-path compare should hit cache"
...
special_third="$(cd "${special_path_repo}" && git_snapshot_test_cmd compare "${special_snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'\tcache_hit_repos=0\tcache_miss_repos=1\t' "${special_third}" "special-path content changes should invalidate cache"
...
head_invalidation_compare="$(cd "${repo}" && git_snapshot_test_cmd compare "${snapshot_id}" --repo . --include-no-effect --porcelain)"
assert_contains $'\tcache_hit_repos=0\tcache_miss_repos=1\t' "${head_invalidation_compare}" "HEAD changes should invalidate cache"
...
assert_contains $'\tcache_hit_repos=0\tcache_miss_repos=1\t' "${missing_first}" "first missing-repo compare should miss cache"

missing_second="$(cd "${nested_root}" && git_snapshot_test_cmd compare "${missing_snapshot_id}" --repo modules/sub1 --porcelain)"
assert_contains $'\tcache_hit_repos=1\tcache_miss_repos=0\t' "${missing_second}" "second missing-repo compare should hit cache"
...
order_first="$(cd "${order_root}" && GIT_SNAPSHOT_COMPARE_JOBS=8 git_snapshot_test_cmd compare "${order_snapshot_id}" --include-no-effect --porcelain)"
order_second="$(cd "${order_root}" && GIT_SNAPSHOT_COMPARE_JOBS=8 git_snapshot_test_cmd compare "${order_snapshot_id}" --include-no-effect --porcelain)"
...
assert_contains $'\tcontract_version=8' "${order_first}" "ordering compare should emit v8 contract summary"
