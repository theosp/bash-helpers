...
assert_contains "Repo summary (changed captures only):" "${inspect_output}" "inspect default should include summary"
...
assert_contains "Details (stat mode):" "${inspect_output}" "inspect default should print stat detail mode"
assert_contains "Repo: super" "${inspect_output}" "inspect default should include root repo detail section"
assert_contains "root.txt |" "${inspect_output}" "inspect stat detail should include tracked file stat output"

inspect_details_output="$(cd "${root_repo}" && git_snapshot_test_cmd inspect "${snapshot_id}" --name-only)"
assert_contains "Details (name-only mode):" "${inspect_details_output}" "inspect --name-only should print detail sections"
